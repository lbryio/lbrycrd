...
static void CheckForkWarningConditions() EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // If our best fork is no longer within 288 blocks (~ 24 blocks per hour +/- 12 hours if no one mines it)
    // of our head, drop it
...
static void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // with a tip within 288 blocks (+/- 12 hours if no one mines it) of ours
    ...
    if (pfork && (!pindexBestForkTip || pindexNewForkTip->nHeight > pindexBestForkTip->nHeight) &&
            ...
            ::ChainActive().Height() - pindexNewForkTip->nHeight < 288)
    {
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!::ChainstateActive().ConnectBlock(block, state, &indexDummy, viewNew, trieCache, chainparams, true))
        return false;
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (nCheckLevel >= 3 && (coins.DynamicMemoryUsage() + ::ChainstateActive().CoinsTip().DynamicMemoryUsage()) <= nCoinCacheUsage) {
            ...
            DisconnectResult res = ::ChainstateActive().DisconnectBlock(block, pindex, coins, trieCache);
    ...
    if (nCheckLevel >= 4) {
        while (pindex != ::ChainActive().Tip()) {
            ...
            if (!ReadBlockFromDisk(block, pindex, chainparams.GetConsensus()))
                return error("VerifyDB(): *** ReadBlockFromDisk failed at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
            if (!::ChainstateActive().ConnectBlock(block, state, pindex, coins, trieCache, chainparams))
