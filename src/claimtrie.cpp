...
bool CClaimTrie::getQueueNameRow(const std::string& name, queueNameRowType& row) const
{
    ...
    if (itQueueNameRow != dirtyQueueNameRows.end())
    {
        ...
        LogPrintf("%s: Found row of size %d for %s\n", __func__, row.size(), name);
        return true;
    }
...
uint256 CClaimTrieCache::getMerkleHash() const
{
    ...
    if (dirty())
    {
        recursiveComputeMerkleHash(getRoot(), "");
    }
...
{
    LogPrintf("%s: nCurrentHeight (before increment): %d, claimQueueCache size %d\n", __func__, nCurrentHeight, claimQueueCache.size());
...
int CClaimTrieCache::getNumBlocksOfContinuousOwnership(const std::string& name) const
{
    ...
    if (node == nullptr || node->claims.empty())
        return 0;
