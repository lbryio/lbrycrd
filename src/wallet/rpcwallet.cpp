...
static void SendMoney(const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew, const char* transferFromAddr)
{
    ...
    vecSend.push_back(recipient);
    if (!pwalletMain->CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet, strError, NULL, true, NULL, -1, transferFromAddr)) {
        if (!fSubtractFeeFromAmount && nValue + nFeeRequired > pwalletMain->GetBalance())
            strError = strprintf("Error: This transaction requires a transaction fee of at least %s because of its amount, complexity, or use of recently received funds!", FormatMoney(nFeeRequired));
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
    }
    if (!pwalletMain->CommitTransaction(wtxNew, reservekey, transferFromAddr == NULL))
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    SendMoney(address.Get(), nAmount, fSubtractFeeFromAmount, wtx, NULL);

    return wtx.GetHash().GetHex();
}

UniValue transferfromaddress(const UniValue& params, bool fHelp)
{
    if (!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "transferfromaddress \"lbrycrdaddress-source\" \"lbrycrdaddress-dest\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
            "\nSend an amount to a given address.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"lbrycrdaddress-source\"  (string, required) The lbrycrd address to send from and receive change to.\n"
            "2. \"lbrycrdaddress-dest\"  (string, required) The lbrycrd address to send to.\n"
            ...
            "\"transactionid\"  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleCli("transferfromaddress", "\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\" \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1")
            + HelpExampleCli("transferfromaddress", "\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\" \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 \"donation\" \"seans outpost\"")
            + HelpExampleCli("transferfromaddress", "\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\" \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 \"\" \"\" true")
            + HelpExampleRpc("transferfromaddress", "\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\" \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", 0.1, \"donation\", \"seans outpost\"")
        );

    LOCK2(cs_main, pwalletMain->cs_wallet);

    CBitcoinAddress srcAddress(params[0].get_str());
    if (!srcAddress.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid LBRY source address");

    CBitcoinAddress destAddress(params[1].get_str());
    if (!destAddress.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid LBRY dest address");
    ...
    SendMoney(destAddress.Get(), nAmount, fSubtractFeeFromAmount, wtx, params[0].get_str().c_str());
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    SendMoney(address.Get(), nAmount, false, wtx, NULL);
...
{ //  category              name                        actor (function)           okSafeMode
    ...
    { "wallet",             "transferfromaddress",      &transferfromaddress,      false },
